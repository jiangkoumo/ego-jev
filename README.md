...
**如果你就是本仓库的作者、会在仓库里直接改代码**，那么：

- **保持软链安装**：`~/.agents/skills/ego-jev` 指向仓库本体（不是拷贝）。改完仓库里的文件**立即生效**，
  不需要任何“更新”动作，也不需要 `git pull`——你的本地仓库就是源头。
- **不要跑 `npx skills add jiangkoumo/ego-jev`**。它是从 GitHub 拉一份**拷贝**：会把这条软链替换成目录，
  之后仓库的改动**不再自动生效**，得先 `commit` + `push` 再重跑它才看得到变化。那是给“只用不改”的用户准备的。
- `./update.sh` 对你是可有可无的（它会显示“已是最新”）；唯一有用的场合是**仓库搬家后重指软链**。

一句话：**作者改仓库，用户跑 update；两种模式不要混。**
