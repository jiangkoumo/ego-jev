# 结论：把 bh 的「滚动算进展」搬进 ego-jev 后，X 上必须连滚 >3 次的深帖任务从 **0/6 变 6/6**（最长连续滚动 5–9 次不再被 `stuck` 中止）、X 合计 14/20→18/18，emoji 截断改为按码点后本地边界用例 21/21 且 X 上 31 次运行 0 次 Jev 400，hn-nav/wiki-search/select-native 配对 bootstrap CI 全部跨 0，无回归。

被测 revision `afe315a`（引擎 md5 `ee62dc5e5d81ac908834996a1ea65bf3`）；改动前 = HEAD `e29c3a9`（md5 `9b760682cebac9fdde7d4172639a928a`，冻结为 `bench/scroll-baseline-engine.mjs`）。下面每个数字都由 `node bench/bh-port-summary.mjs` 从 `bench/raw/` 原始文件重算，不是手抄。

## P0-1 滚动感知的进展判定（依据：REVEAL-REPORT ①②③——0/15、`stuck` 12 次、B 的 `fingerprint()` 含 scroll）
实现：进展 = URL/导航变化 **或** 滚动动作真的移动了视口（`scrollMoved`，动作后实测；退回鼠标滚轮时再读一次 `scrollY`）**或** 上一步是滚动且本轮观测露出新元素（`newTargetCount`，观测层本来就在算）。有界性没放开：到页面边界后视口不再移动、也不再露新元素，`sameActionStreak` 照常累加到 `maxSameAction` 停止，`maxSteps` 仍是硬上限——`bench/test-scroll-progress.mjs` 24/24（[2] 边界必须停、[5] 滚入不成环）。
...
`bench/x-scroll-pair.js`：4 任务 × 2 引擎 × 6 轮，配对、奇偶轮翻转先后、只读互锁拦截 0 次；失败原因分布 old `{stuck:6}` / new `{}`。
...
node bench/test-scroll-progress.mjs               # 24/24（无需浏览器）
...
```
原始数据：`bench/raw/` 下 `x-scroll-pair-*`、`x-frozen-new-*`、`x-one-step-*`、`x-diagnose-*`、`x-emoji-audit-*`、`scroll-noregress-*`、`delayed-nav-2026-09-22T09-02-33-690Z.json`、`test-emoji-clip-*`、`test-scroll-progress-*`、`test-guardrails-2026-09-22T09-22-25.txt`。
