...
> ⚠️ **已知不可用场景，先看这条**（实测数据见仓库 `REVEAL-REPORT.md`、`VERIFY-REPORT.md`）
>
> - **X（twitter.com）等虚拟化无限滚动站点：不要用本技能。** 实测 24 轮 **0/15**（连“目标就在首屏
>   0.26 屏”的任务也是 0/5），失败原因首位是 `stuck`（12 次）。根因：引擎判定「页面是否变化」**只看 URL**，
>   而滚动不改 URL → 连续 3 次滚动即中止。这类任务改走 **browser-harness（`bh`）**：同样任务上 6/12
>   （它的指纹包含滚动位置，因此不会被中止）。本技能适用于**目标唯一且已在视口内**的点击/搜索/表单页。
> - **emoji 密集页面会随机硬失败**：元素名/值用 UTF-16 `slice(0,60)` 截断，可能切开 emoji 产生孤立代理
>   → Jev 返回 400 `invalid Unicode text` → **整轮 `action_failed`**（X 时间线 235 个元素里约 5 个命中）。**未修复**。
> - **「有界滚动揭示」在真实站点 24 轮 0 次触发**：它只在「选了需要目标的动作却解析不出目标」时生效；
>   真实长列表里 Jev 会自己滚或点相似链接，不会选 `none`——不要指望它解决“目标在视口外”。
